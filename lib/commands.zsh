...
# Generate default .viban/workflow.md template
cmd_init() {
    local workflow_file="$VIBAN_DATA_DIR/workflow.md"

    if [[ -f "$workflow_file" ]]; then
        echo "⚠ .viban/workflow.md already exists — skipping."
        echo "  Edit it directly or delete it and re-run: viban init"
        return 0
    fi
...
> Auto-applied by `/viban:assign`. Generated by `viban init` - edit freely.

## Pipeline

Analyze → Implement → Verify → Build → Commit → PR → Review (fully autonomous)
...
All work MUST happen in an isolated git worktree.
...
- All file reads, edits, builds, and tests happen inside `$WT_DIR`
- Do NOT remove the worktree after shipping — it stays for review
...
Build/tests must pass + manual verification of the fix.
...
```bash
# Replace with your project's build/test command
npm test
```
...
### Branch Convention
`issue-{id}` (e.g. `issue-3`, `issue-4`)

### Pull Request
Auto-create PR and move issue to review via `viban review {id}`.
...
- Issue tracking: auto-increment (viban default)
...
WORKFLOW_EOF
    ...
    echo "✓ Created .viban/workflow.md"
    echo "  /viban:assign will now follow your custom workflow."
    echo "  Edit .viban/workflow.md anytime to adjust it."
}
